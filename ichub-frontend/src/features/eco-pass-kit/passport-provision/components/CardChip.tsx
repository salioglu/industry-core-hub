...
const statusStyles: Record<string, ChipStyle> = {
  active: {
    color: 'registered',
...
export const CardChip = ({ status, statusText, className }: CardChipProps) => {
  ...
  return (
    <MuiChip
      ...
      className={className}
      sx={{
        color: statusKey === 'shared' ? '#000000' : theme.palette.chip[color],
      ...
      icon={statusKey === 'shared' ? <PersonIcon sx={{
        color: '#000000',
        fontSize: '18px'
      }} /> : undefined}
    />
